...
export const CompositionSpec=z.object({videoPath:z.string().min(1).max(4096),additionalVideoPaths:z.array(z.string().min(1).max(4096)).max(19).optional(),videoEdits:z.array(CompositionVideoEdit).max(20).optional(),audioPath:z.string().min(1).max(4096).optional(),audioVolume:z.number().min(0).max(2).default(1),logos:z.array(CompositionLogo).max(8).optional(),destinationDir:z.string().min(1).max(4096),outputName:z.string().trim().max(120).optional()}).strict().superRefine((value,context)=>{const videos=[value.videoPath,...(value.additionalVideoPaths??[])];const logos=value.logos??[];if((value.audioPath&&videos.includes(value.audioPath))||logos.some(logo=>videos.includes(logo.path)||(value.audioPath!==undefined&&logo.path===value.audioPath)))context.addIssue({code:"custom",message:"Video, audio and logo inputs must be different"});if(!value.videoEdits&&new Set(videos).size!==videos.length)context.addIssue({code:"custom",message:"Video inputs must be unique unless they are timeline segments"});if(value.videoEdits&&(value.videoEdits.length!==videos.length||value.videoEdits.some((edit,index)=>edit.path!==videos[index])))context.addIssue({code:"custom",message:"Video edits must match the ordered video inputs"});});
