...
export const ipcContract = {
  ...
  "dialog:composition-logo": { input: z.object({}).strict().default({}), output: z.string().nullable() },
  ...
  "compositions:inspect-file": { input: z.object({path:z.string().min(1).max(4096)}).strict(), output: z.object({duration:z.number().nonnegative(),width:z.number().int().nonnegative(),height:z.number().int().nonnegative(),kinds:z.array(z.string())}) },
  "compositions:waveform": { input: z.object({path:z.string().min(1).max(4096)}).strict(), output: z.object({path:z.string().min(1)}) },
