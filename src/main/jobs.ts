...
export class JobManager extends EventEmitter {
  ...
  settings(): AppSettings { const defaults={downloadDir:path.join(os.homedir(),"Downloads","Super Video Pro"),concurrency:2,perDomainConcurrency:1,cookiesFromBrowser:"none" as const,historyRetentionDays:90,logRetentionDays:30,backupRetentionCount:3};const stored=this.db.getSetting<Record<string,unknown>>("app",{});const parsed=AppSettings.safeParse({...defaults,...stored});if(parsed.success)return parsed.data;this.db.quarantineSetting("app",stored,parsed.error);return AppSettings.parse(defaults); }
