...
async function createWindow(){
  ...
  const debug=await win.webContents.executeJavaScript(`(async()=>({location:location.href,html:document.body.innerHTML,bridge:typeof window.superVideo,ready:document.readyState,selectedTab:[...document.querySelectorAll('[data-tab]')].find(tab=>tab.getAttribute('aria-selected')==='true')?.getAttribute('data-tab'),selectedTabBackground:getComputedStyle([...document.querySelectorAll('[data-tab]')].find(tab=>tab.getAttribute('aria-selected')==='true')).backgroundColor,update:await window.superVideo.invoke('updates:check',{})}))()`);
