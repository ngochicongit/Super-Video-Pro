...
async function createWindow(){
  ...
  const rendererEvents:string[]=[];win.webContents.on("console-message",(_event,level,message)=>rendererEvents.push(`console:${level}:${message}`));win.webContents.on("render-process-gone",(_event,details)=>rendererEvents.push(`gone:${details.reason}:${details.exitCode}`));win.on("unresponsive",()=>rendererEvents.push("window:unresponsive"));
  ...
  await fs.writeFile(`${process.env.SVP_SCREENSHOT_PATH}.json`,JSON.stringify({...debug,downloadStatus,editorPlaybackStatus,rendererEvents},null,2));
