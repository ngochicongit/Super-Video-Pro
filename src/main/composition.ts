...
import {parseFfmpegProgressLine} from "./ffmpeg-progress.js";
...
export class CompositionManager extends EventEmitter{
  ...
  private async execute(initial:Job){
    ...
    try{
      ...
      const progressArgs=[...args.slice(0,-1),"-progress","pipe:1","-nostats",args.at(-1)!];let lastPercent=5;
      const result=await runTool("ffmpeg",progressArgs,{signal:controller.signal,onLine:line=>{const elapsed=parseFfmpegProgressLine(line);if(elapsed===null||!videoDuration)return;const next=Math.min(89,Math.max(5,Math.round((elapsed/videoDuration)*84)+5));if(next<=lastPercent)return;lastPercent=next;const current=this.db.getComposition(job.id);if(current?.status==="processing")job=this.save({...current,progress:next/100});}});if(result.code!==0)throw Object.assign(new Error(result.stderr.trim()||"FFmpeg composition failed"),{code:"COMPOSITION_PROCESSING"});
