...
export function registerIpc(jobs: JobManager,diagnostics:Diagnostics,updater:AppUpdater,evidence:ProductEvidence) {
  const handlers={...appHandlers(jobs,diagnostics,evidence),...jobHandlers(jobs),...runtimeHandlers(updater)};
