...
export function logoOverlayFilter(logos:LogoOverlay[],videoLabel="vbase",startIndex=0){
  return logos.map((logo,index)=>{
    const input=startIndex+index;const source=index?`logoout${index-1}`:videoLabel;const color=(logo.backgroundColor??"#000000").replace("#","0x"),borderColor=(logo.borderColor??"#ffffff").replace("#","0x");
    const chain=[`scale='min(${logo.width},iw)':-1`,logo.hue?`hue=h=${logo.hue}`:"null","format=rgba",logo.borderWidth?`pad=iw+${logo.borderWidth*2}:ih+${logo.borderWidth*2}:${logo.borderWidth}:${logo.borderWidth}:color=${borderColor}@1`:"null",logo.padding?`pad=iw+${logo.padding*2}:ih+${logo.padding*2}:${logo.padding}:${logo.padding}:color=${color}@${logo.backgroundOpacity??0}`:"null",`colorchannelmixer=aa=${logo.opacity}`,logo.staticEffect==="fade-in"?`fade=t=in:st=0:d=${logo.fadeDuration??1}:alpha=1`:"null"].join(",");
