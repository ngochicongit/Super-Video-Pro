...
if errorlevel 1 .venv\Scripts\python.exe -m pip install "pyinstaller>=6.22,<7"
if errorlevel 1 exit /b 1
.venv\Scripts\python.exe -m compileall -q packages
if errorlevel 1 exit /b 1
echo [Super Video Pro] Running verification, backend bundling, and NSIS build...
call %PNPM_CMD% package
if errorlevel 1 (
  echo Build failed.
  exit /b %errorlevel%
)
