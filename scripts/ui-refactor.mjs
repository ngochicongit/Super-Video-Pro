...
async function run(file,args=[],env={}){await new Promise((resolve,reject)=>{const child=spawn(file,args,{cwd:root,env:{...process.env,...env},stdio:"inherit",shell:process.platform==="win32"});child.on("error",reject);child.on("exit",code=>code===0?resolve():reject(new Error(`${file} exited with ${code}`)));});}
